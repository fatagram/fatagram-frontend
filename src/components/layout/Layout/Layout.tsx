...
const LayoutHeader: React.FC<LayoutHeaderProps> = ({ children, className }) => {
  return <header className={`fixed z-40 w-full ${className}`}>{children}</header>;
};
...
const LayoutMain: React.FC<LayoutMainProps> = ({ children, className }) => {
  return <main className={`relative h-full ${className}`}>{children}</main>;
};
...
const LayoutFooter: React.FC<LayoutFooterProps> = ({ children, className }) => {
  return (
    <footer className={`sm:hidden flex fixed z-40 bottom-0 w-full ${className}`}>{children}</footer>
  );
};
