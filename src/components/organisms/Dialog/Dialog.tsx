...
const Dialog: React.FC<DialogBoxProps> = ({
  ...
  return (
    <div
      className={`relative flex flex-col gap-4 bg-[var(--second-bg-color)] 
                        rounded-lg shadow-lg ${className}`}
    >
      {title && (
        <Text weight="bold" size="lg-2">
          {title}
        </Text>
      )}
      ...
      <Text
        className={`absolute top-3 right-5 text-[20px] text-gradient-main hover:text-single-main 
                                cursor-pointer`}
        onClick={onClose}
      >
