    ...
    async (error) => {
        if (error.response?.status === 401 || error.response?.status === 403) {
            try {
                ...
                const refreshResult = await axios.post(`${appConfig.apiUrl}/api/auth/refreshToken`, 
    ...
    async (error) => {
        if (error.response?.status === 401 || error.response?.status === 403) {
            try {
                // Refresh token
                const refreshToken = getRefreshToken() || getRefreshTokenFromSession();
                const refreshResult = await axios.post(`${appConfig.apiUrl}/api/auth/refresh-token`, 
                    { refreshToken: refreshToken }, 
                    { withCredentials: true })
